...
patterns = {heart;word};
currentPosition = p0 + Rot*[0;0;-0.02];% current end-effector position 
currentTheta = Solution(:,4).';% current joint angles
for item = 1 : size(patterns,1)% for each pattrn
    % move to the new point with xxcm height
    nextPosition = p0 + Rot*((patterns{item,1}(1,:)/100).'+[0;0;-height]);% goal end-effector position
    V = [nextPosition - currentPosition;0;0;0];
    J = getJacobian(currentTheta);% Current Jacoobian
    nextTheta = currentTheta + ((inv(J))*V).';% next moment joint angles
    currentPosition = nextPosition;
    currentTheta = nextTheta;
    move_robt(id,vrep,h,nextTheta,Socket_conn,REAL);
    pause(2)
    %
    for i = 1 : size(patterns{item,1},1)% for each point in the current pattern
        nextPosition = p0 + Rot*((patterns{item,1}(i,:)/100).');
        V = [nextPosition - currentPosition;0;0;0];
        J = getJacobian(currentTheta);
        nextTheta = currentTheta + ((inv(J))*V).';
        currentPosition = nextPosition;
        currentTheta = nextTheta;
        move_robt(id,vrep,h,nextTheta,Socket_conn,REAL);
        pause(0.1)
        ...
        if i == size(patterns{item,1},1)% move up
            pause(2)
            nextPosition = p0 + Rot*((patterns{item,1}(i,:)/100).'+[0;0;-height]);
            V = [nextPosition - currentPosition;0;0;0];
            J = getJacobian(currentTheta);
            nextTheta = currentTheta + ((inv(J))*V).';
            currentPosition = nextPosition;
